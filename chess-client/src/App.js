...
const ChessBoard = () => {
  ...
  const [history, setHistory] = useState([]); // Track the history of moves
  ...
  useEffect(() => {
    setChess(new Chess());
    setHistory([{ position: 'start', move: '' }]);
  ...
  const makeMove = async (fen, move, difficulty) => {
    try {
      ...
      setGameStatus('Game in Progress...');
      setHistory((prevHistory) => [...prevHistory, { position: newPosition, move: response.data.stockfish_move }]);
  ...
  const onDrop = async (sourceSquare, targetSquare) => {
    ...
    console.log(currentMoveIndex, history);
    setHistory((prevHistory) => [...prevHistory, { position: newPosition, move: move_string }]);
    console.log(currentMoveIndex, history);
    setCurrentMoveIndex(history.length);
